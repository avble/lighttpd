...
static int connection_handle_read(server *srv, connection *con) {
	...
	b->ptr[b->used++] = '\0';
        ...
        printf("[Debug] %s, %d, header request: \n %s  \n", __FILE__, __LINE__, b->ptr + read_offset);

#if 0
        // replace GET by HEAD if any
        if (strstr(b->ptr + read_offset, "Range") == NULL)
        {
            int read_offset2 = b->used - 1; // read_offset2 is at last NULL character

            int index = 0;
            while(index <= len-2)
            {
                b->ptr[read_offset2 - index + 1] = b->ptr[read_offset2 - index];
                index++;
              //  printf("[Debug] %s, %d \n", __FILE__, __LINE__);
            }
            b->ptr[read_offset + 0] = 'H';
            b->ptr[read_offset + 1] = 'E';
            b->ptr[read_offset + 2] = 'A';
            b->ptr[read_offset + 3] = 'D';
            b->ptr[read_offset + 4] = ' ';
        ...
        printf("[Debug] %s, %d, header request: %s  \n", __FILE__, __LINE__, b->ptr + read_offset);

#endif

        //printf("[Debug] %s, %d, request: %s \n", __FILE__, __LINE__, b->ptr + read_offset);
...
static int connection_handle_write_prepare(server *srv, connection *con) {
	...
	default: /* class: header + body */
            printf("[Debug] %s, %d  http status: %d \n", __FILE__, __LINE__, con->http_status);
...
connection *connection_accept(server *srv, server_socket *srv_socket) {
	...
	printf("[Debug] %s, %d  error \n", __FILE__, __LINE__);
        ...
        printf("[Debug] %s, %d  number of connection: %d \n", __FILE__, __LINE__, srv->con_opened);
		srv->cur_fds++;
