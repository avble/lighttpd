...
static int request_check_hostname(server *srv, connection *con, buffer *host) {
	...
	int level = 0;
        ...
        printf("[Debug] %s, %d: server: %X, connection: %X \n",  __FILE__, __LINE__, srv, con);
...
int http_request_parse(server *srv, connection *con) {
	...
	in_folding = 0;
        ...
        printf("[Debug] %s, %d, URI: %s \n", __FILE__, __LINE__, con->request.uri->ptr);
#if 1
        // HACK
        if (strstr(con->request.uri->ptr, "stream-"))
        {
        char uri_full[256];
        strcpy(uri_full, con->request.uri->ptr);

         char *p1 = strchr(uri_full, '-');
         assert(p1 != NULL);
         char *p2 = strchr(uri_full + 2, '.');
         assert(p2 != NULL);

         printf("[Debug] p1: %s, p2: %s \n", p1, p2);

         char s_number[256];
         int index = 0;
         while(p1 + index + 1 < p2)
         {
             s_number[index] = *(p1 + index + 1);
             printf("number at index: %x is :%x \n", s_number[index]);
             index++;
         }
         s_number[index] = NULL;
         ...
         *(p1 + 1) = NULL;
         ...
         sprintf(nx_uri, "%s%d.ts", uri_full, number + 1);


         char full_path[256];
         strcpy(full_path, "/var/www/html");
         strcat(full_path, nx_uri);

           printf("[Debug] %s, %d, Waiting for URI: %s, new uri: %s  number: %s\n", __FILE__, __LINE__, full_path, nx_uri, s_number);
             struct stat stat_;
_RESTART_:
             if (stat(full_path, &stat_) == -1)
             {
