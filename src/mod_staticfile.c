...
//        physical.path
      ...
      //  buffer_copy_string(con->physical.path, "/var/www/html/channel22.ts");
      //  printf("[Debug] %s, %d \n", __FILE__, __LINE__);
...
//			con->response.content_length += end - start + 1;
			...
			con->response.content_length += end - start + 1;
                        ...
                        {
                            buffer_copy_string(con->physical.path, "/var/www/html/channel2.big.big.ts");
                            if (HANDLER_ERROR == stat_cache_get_entry(srv, con, con->physical.path, &sce)) {
                                    SEGFAULT();
                           }
                            end = sce->st.st_size - 1;
                        ...
                        // me: debugging
                        printf("[Debug] %s, %d, start: %d, len: %d \n", __FILE__, __LINE__, start, end - start + 1);
